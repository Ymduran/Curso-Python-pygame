...
class SnakeBlock(Sprite):
    ...
    def __init__(self, is_head: bool = False):
        ...
        for i in range(len(Configurations.get_apple_images_path())):
            ...
            frame = pygame.transform.scale(frame, (Configurations.get_snake_block_size(), Configurations.get_snake_block_size()))
        ...
        if is_head:image = Configurations.get_snake_heads_path()
        else:
            body_images_path = Configurations.get_snake_body_image_path()
            image = choice(body_images_path)
        ...
        snake_size = Configurations.get_apple_size()
    ...
    def blit(self, screen: pygame.surface.Surface) -> None:
        ...
        angle = 0
        if SnakeBlock.get_is_moving_up(): angle = 90
        elif SnakeBlock.get_is_moving_left(): angle = 180
        elif SnakeBlock.get_is_moving_down(): angle = 270
    ...
    def animate_head(self)-> None:
        ...
        if needs_refresh:
            self.image = self.head_frames[self._frame_index]

            self._last_update_time = current_time
            self._frame_index += 1

            if self._frame_index >= len(self.head_frames): self._frame_index = 0
